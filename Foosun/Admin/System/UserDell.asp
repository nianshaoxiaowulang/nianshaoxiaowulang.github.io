...
if Request.Form("action")="Submit" then
	...
	If OperateType = "Dell" then
		if Not JudgePopedomTF(Session("Name"),"P040403") then Call ReturnError()
		Dim TempRs
		Set TempRs=Conn.execute("Select MemName from FS_Members where ID in (" & UserIDP & ")")
		Do while not Temprs.eof
			Conn.Execute("Delete from FS_Message where MeRead='"&Temprs(0)&"'")
			Temprs.MoveNext
		Loop
		Conn.Execute("Delete from FS_Members where ID in (" & UserIDP & ")")
		Set TempRs=Nothing
	Elseif OperateType = "Lock" then
		if Not JudgePopedomTF(Session("Name"),"P040405") then Call ReturnError()
		Conn.Execute("Update FS_Members set Lock=1 where ID in (" & UserIDP & ")")
	Else
		if Not JudgePopedomTF(Session("Name"),"P040405") then Call ReturnError()
		Conn.Execute("Update FS_Members set Lock=0 where ID in (" & UserIDP & ")")
	End If
