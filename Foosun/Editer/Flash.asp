        ...
        <TABLE border="0" cellpadding="0" cellspacing="3" >
          <TR>
            <TD height="17" >地址： <INPUT name="url" id=url value="http://" size=30>
              <input type="button" name="Button" value="选择动画" onClick="var TempReturnValue=OpenWindow('../FunPages/SelectPic.asp?LimitUpFileFlag='+'<% = LimitUpFileFlag %>'+'&CurrPath=/<% = UpFiles %>',500,290,window);if (TempReturnValue!='') document.all.url.value='<% = DirectoryRoot %>'+TempReturnValue;" class=Anbutc> 
            </td>
          </TR>
          <TR>
            <TD >宽度：
